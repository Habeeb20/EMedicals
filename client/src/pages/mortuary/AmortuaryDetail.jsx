...
//         axios.get(`${import.meta.env.VITE_API_m}/mortuarydetails/${id}/comments`)
//             .then((response) => {
...
//         if (!newComment || !userEmail) return;
...
//                     <h6 className="font-bold ">Addresss: {leader.address}</h6>
//                     <h6 className="font-bold ">unique number: {leader.uniqueNumber}</h6>
//                     <h6 className="font-bold ">years in Profession: {leader.yearsInProfession} years</h6>
...
//                 {["Cemetery", "Burial Service", "Cremation", "Casket", "Pregnancy"].map((service, index) => (
//                     <span key={index} className="bg-green-200 text-green-700 px-3 py-1 rounded-full text-sm my-1">
//                         {service}
//                     </span>
//                 ))}
//             </div>
...
//                 <button className="text-green-600 font-semibold mt-2">Contact: </button>
...
//                     <div
//                         key={index}
//                         className={`p-2 text-center rounded-lg ${item.active ? "bg-green-600 text-white" : "bg-gray-100 text-gray-600"}`}
//                     >
//                         <p>{item.day}</p>
//                         <p>{item.date}</p>
//                     </div>
...
//             {/* Booking Times */}
//             <div className="flex flex-wrap space-x-2 mt-6">
//                 {["09:00 AM", "10:00 AM", "11:00 AM", "01:00 PM", "02:00 PM", "04:00 PM"].map((time, index) => (
//                     <span key={index} className="bg-gray-100 text-gray-600 px-3 py-1 rounded-lg text-sm mt-2">
//                         {time}
//                     </span>
//                 ))}
//             </div>
...
//                             <div key={comment._id} className="p-3 bg-gray-100 rounded-lg shadow-md">
...
//                 <button
//                     type="submit"
//                     className={`mt-2 w-full py-2 rounded-md ${loading ? "bg-gray-400" : "bg-green-600 text-white"}`}
//                     disabled={loading} 
//                 >
...
const AmortuaryDetail = () => {
    ...
    return (
        <div className="p-4 md:p-8 max-w-full mx-auto bg-gray-50 min-h-screen">
            ...
            {/* Calendar */}
            <div className="flex space-x-2 mt-6 overflow-x-auto">
                {[{ day: "Mon", date: "21" }, { day: "Tue", date: "22" }, { day: "Wed", date: "23", active: true }, { day: "Thu", date: "24" }, { day: "Fri", date: "25" }, { day: "Sat", date: "26" }].map((item, index) => (
                    <div key={index} className={`p-2 text-center rounded-lg ${item.active ? "bg-green-600 text-white" : "bg-gray-100 text-gray-600"}`}>
                        <p>{item.day}</p>
                        <p>{item.date}</p>
                    </div>
                ))}
            ...
            <div className="mt-6">
                ...
                {comments.length > 0 ? (
                    <div className="mt-4 space-y-4">
                        {comments.map((comment) => (
                            <div key={comment._id} className="p-3 bg-gray-100 rounded-lg shadow-md">
                                <p className="text-sm font-semibold text-gray-800">{comment.userName}</p> {/* Updated to access userName */}
                                <p className="text-gray-600 text-sm">{comment.commentText}</p> {/* Updated to access commentText */}
                            </div>
                        ))}
                    </div>
                ) : (
