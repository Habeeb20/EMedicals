...
const DeathRecordForHospital = () => {
  ...
  const [showFormForDeath, setShowFormDeath] = useState(false)
  ...
  const handleInputChangeForDeath = (e) => {
...
///post death record 
const handleSubmitForDeathRecord = async() => {
  try {
    const token = localStorage.getItem("token")
    const response =await axios.post(`${import.meta.env.VITE_API_HO}/addDeceased`, deathData, {
      headers:{
        Authorization:`Bearer ${token}`
