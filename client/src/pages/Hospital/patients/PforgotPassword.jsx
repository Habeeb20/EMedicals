...
const PforgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${import.meta.env.VITE_API_HO}/forgot-password`, { email });
      toast.success("Password reset email sent");
      setSuccess("email has been sent sucessfully")
        navigate('/preset-password/:token')
    } catch (err) {
      console.log(err);
  ...
  return (
    <div className="min-h-screen flex justify-center items-center bg-gray-100">
      <form onSubmit={handleSubmit} className="w-full max-w-md bg-white p-8 rounded-lg shadow-md">
        <h2 className="text-2xl font-semibold text-center mb-6">Forgot Password(patient)</h2>
