...
const NurseDashboardHospital = () => {
    ...
    const fetchProfile = async () => {
      try {
        ...
        const { data } = await axios.get(
          `${import.meta.env.VITE_API_HO}/getnursedashboard3`,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(
        `${import.meta.env.VITE_API_HO}/editnurse/${userId}`,
        userData,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
...
export default NurseDashboardHospital;
