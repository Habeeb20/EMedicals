    ...
    const verifyPayment = async () => {
      try {
        ...
        const response = await axios.get(
          `${import.meta.env.VITE_API_T}/paystack/verify?reference=${reference}`,
        ...
        if (response.status === 200) {
          ...
          navigate("/telepayment-failed");
        }
      } catch (error) {
        console.error("error!!!", error);
        setErrorMessage("Payment verification failed.");
        navigate("/telemakepayment");
      } finally {
        setVerifying(false);
      }
    };
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-600">Please don't close this window, you will be redirected shortly</p>
        </div>
      </div>
    );
  }
...
export default TeleVerifyPayment;
