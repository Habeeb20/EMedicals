...
const PatientDetails = ({ appointmentId }) => {
  ...
  useEffect(() => {
    const fetchPatientDetails = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_A}/doctor/${appointmentId}}`);
        setPatient(response.data.patient); // Assuming the patient details are returned in this format
        setLoading(false);
      } catch (err) {
        setError(err.response ? err.response.data.msg : 'Error fetching patient details');
        setLoading(false);
      }
    };
  ...
  return (
    <div className="patient-details">
      ...
      <div className="details">
        ...
        <p><strong>Drugs Taken:</strong> {patient.drugsTaken}</p>
        <p><strong>Appointment Started:</strong> {patient.started}</p>
      </div>
    </div>
  );
};
